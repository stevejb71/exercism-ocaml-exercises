...
let s7 = lazy (CSet.add (CSet.empty) 7)
let assert_not_equal msg s1 s2 _test_ctxt = assert_bool msg ((CSet.equal s1 s2) |> not)
...
let tests = [
    ...
    "empty != node">::
        assert_not_equal "empty != node" CSet.empty (CSet.add (CSet.empty) 7);
    "node != empty">::
        assert_not_equal "node != empty" (CSet.add (CSet.empty) 7) CSet.empty;
    "equal - different non-empty sets">::
        assert_not_equal "equal - different non-empty sets" (CSet.add (CSet.empty) 7) (CSet.add (CSet.empty) 8);
